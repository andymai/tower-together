...
- **Minimap.tsx** — SimTower-style draggable Map window. Title bar with Map label + state-aware right button (✕ close / ▲ restore), collapses window-shade-style to title bar only. Canvas height is derived from world aspect (`GRID_WIDTH × TILE_WIDTH` : `GRID_HEIGHT × TILE_HEIGHT`) so the building's proportions match the in-game view; sky/dirt regions are color-tinted (the transition is the ground plane, no extra line). Two tabs (Edit silhouette / Eval color-coded by evalLevel), draggable yellow viewport rectangle, click/drag inside the canvas to fast-travel. Persists collapsed state, position, and active tab per-tower.
