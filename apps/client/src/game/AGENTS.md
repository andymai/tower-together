...
- **GameScene.ts** — Main Phaser `Scene`. Renders tower grid, sims, elevator cars; owns pointer/hover interaction state including build previews, inspect clicks, and the canvas cursor; exposes `apply*` methods for state updates from `GameScreen`.
