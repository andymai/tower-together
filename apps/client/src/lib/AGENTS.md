...
- **lockstepSession.ts** — Client-side lockstep controller. Runs a local `TowerSim`, predicts local input batches, replays from authoritative batches plus checkpoints, caches materialized sim state records, and emits render updates to `useTowerSession`.
