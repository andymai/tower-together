...
### `carriers.ts`
`CarrierRecord` and `CarrierCar` state machine. `floor_to_slot(carrier, floor)` maps floor index to car waiting-count slot: modes 0/1 (local-mode) use up to 10 regular slots + sky-lobby slots; mode 2 (Service/express) uses direct offset. `make_carrier(..., numCars?)` now supports up to 8 active cars with distributed home floors. `rebuild_carrier_list(world)` scans **only elevator** overlays (escalators are special-link segments, not carriers) and rebuilds `world.carriers`. `tick_all_carriers(world, time)` uses daypart/calendar state for schedule gating, assigns floor requests to the best car in a shaft, keeps per-floor directional route status arrays in sync, boards only routes owned by that car, and drives the spec-shaped departure / dwell flow instead of the earlier pure wait-count SCAN loop.

### `routing.ts`
Routing infrastructure. `rebuild_special_links(world)` populates `world.special_links` from elevator carriers (bit 0 = 1 for mode-2/express) AND from escalator overlays (grouped by column, local-mode, `carrier_id = -1`). `rebuild_walkability_flags(world)`, `rebuild_transfer_group_cache(world)` recompute derived state. The transfer cache is no longer a blanket "served floors" mask: it is built only from explicit family-24 transit-concourse/parking objects, with mode-dependent carrier tolerance. `is_floor_span_walkable_for_local_route` / `_for_express_route` enforce the recovered span/gap limits. `select_best_route_candidate(world, from, to, preferLocalMode?)` applies mode-aware carrier filtering plus `0x28` source-floor congestion penalties for direct and transfer elevator routes.
