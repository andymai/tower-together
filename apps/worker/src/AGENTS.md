...
- **protocol.ts** — Wire-protocol helpers. Parses raw WebSocket payloads into `ClientMessage`, identifies session-only messages including debug-only runtime overrides, and maps gameplay messages onto sim-level `SimCommand` values.
...
- **types.ts** — Shared types and game constants: `TowerSave`, `TowerRuntimeState`, `ClientMessage`/`ServerMessage` unions, and the server-to-client HUD payloads for cash plus star rating, along with `TILE_WIDTHS`, `TILE_COSTS`, `HOTEL_DAILY_INCOME`, `VALID_TILE_TYPES`, `TICKS_PER_DAY`, `STARTING_CASH`.
