...
### `apps/client`
React 18 + Vite + TypeScript + Phaser 3 frontend. Handles guest login, tower lobby, and the game screen with a Phaser-rendered grid canvas. Communicates with the backend via HTTP (tower create/join) and WebSocket (realtime tile edits + sim clock), and renders top-level HUD state such as cash and tower star rating from server broadcasts.
